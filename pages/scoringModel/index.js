...
export default function Home({ data }) {
    ...
    const handleChangeNumber = (event) => {
        ...
        if (name === 'lowSatisfaction') {
            if (newValue >= 0 && newValue <= 100) {
                setValues({ ...values, [name]: newValue });
            }
        } else if (name === 'timeoutSetting') {
            if (newValue >= 0) {
                setValues({ ...values, [name]: newValue });
            }
        } else if (name === 'shiftTimeoutSetting') {
            if (newValue >= 0) {
                setValues({ ...values, [name]: newValue });
            }
        } else if (name === 'claimRepetition') {
            if (newValue >= 0) {
                setValues({ ...values, [name]: newValue });
            }
        } else if (name === 'demeritLimit') {
            if (newValue >= 0) {
                setValues({ ...values, [name]: newValue });
            }
        }
        else if (name === 'lowSatisfactionSingleDeduction') {
            if (newValue <= 0) {
                setValues({ ...values, [name]: newValue });
            }
        }
        else if (name === 'overtimeSingleDeduction') {
            if (newValue <= 0) {
                setValues({ ...values, [name]: newValue });
            }
        } else if (name === 'shiftSingleDeduction') {
            if (newValue <= 0) {
                setValues({ ...values, [name]: newValue });
            }
        } else if (name === 'repeatedEventSingleDeduction') {
            if (newValue <= 0) {
                setValues({ ...values, [name]: newValue });
            }
        }
